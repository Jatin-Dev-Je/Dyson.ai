...
const sources: Source[] = [
  { id: 'slack',   name: 'Slack',   status: 'connected',    detail: 'acme-corp.slack.com Â· 24 channels',    lastSync: '2 min ago',  events: '12.4k',  icon: MessageSquare, color: '#E01E5A' },
  { id: 'github',  name: 'GitHub',  status: 'connected',    detail: 'acme-corp Â· 8 repositories',           lastSync: '4 min ago',  events: '8.1k',   icon: Github,        color: '#8B949E' },
  { id: 'notion',  name: 'Notion',  status: 'error',        detail: 'Token expired â€” reconnect required',    lastSync: '3 days ago', events: '2.3k',   icon: FileText,      color: '#ffffff' },
  { id: 'linear',  name: 'Linear',  status: 'disconnected', detail: 'Not connected',                        lastSync: 'â€”',          events: 'â€”',       icon: Calendar,      color: '#5E6AD2' },
]
...
const statusCfg: Record<Status, { badge: string; dot: string; label: string }> = {
  ...
  disconnected: { badge: 'text-ink-3 bg-subtle border-line',  dot: 'bg-white/20',   label: 'Not connected' },
}
...
export default function ConnectedSources() {
  ...
  return (
    <div className="px-10 py-8 max-w-[720px]">
      <div className="mb-8">
        <h1 className="text-[20px] font-semibold text-ink-1 mb-1">Connected sources</h1>
        ...
        {sources.map((source, i) => {
          ...
          return (
            <motion.div
              ...
              className="rounded-lg border border-line bg-surface p-5 hover:border-line-strong transition-colors"
            >
              <div className="flex items-center gap-4">
                <div className="w-10 h-10 rounded-xl bg-subtle border border-line flex items-center justify-center flex-shrink-0">
                  <Icon className="w-5 h-5" style={{ color: source.color }} />
                </div>

                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2.5 mb-1">
                    <span className="text-[14px] font-medium text-ink-1">{source.name}</span>
                ...
                <div className="flex items-center gap-2 flex-shrink-0">
                  {status === 'connected' && (
                    <>
                      <div className="text-right mr-2">
                        <p className="text-[11px] font-mono text-ink-4">{source.events} events</p>
                        <p className="text-[10px] text-ink-4">synced {source.lastSync}</p>
                      </div>
                      <button
                        onClick={() => handleSync(source.id)}
                        className="p-2 rounded-lg hover:bg-hover text-ink-3 hover:text-primary transition-all"
                        title="Sync now"
                      >
                      ...
                      <button
                        ...
                        className="p-2 rounded-lg hover:bg-red-500/10 text-ink-3 hover:text-red-400 transition-all"
                        title="Disconnect"
                      >
                  ...
                  {status === 'disconnected' && (
                    <button
                      onClick={() => handleConnect(source.id)}
                      className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg border border-line text-[12px] text-ink-3 hover:text-ink-1 hover:border-primary/40 hover:bg-primary/10 transition-all"
                    >
              ...
              {isSyncing && (
                <div className="mt-3 pt-3 border-t border-line">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[10px] font-mono text-ink-3">Syncingâ€¦</span>
                  </div>
                </div>
              )}
            </motion.div>
      ...
      <div className="mt-6 p-4 rounded-xl border border-line bg-subtle flex items-start gap-3">
