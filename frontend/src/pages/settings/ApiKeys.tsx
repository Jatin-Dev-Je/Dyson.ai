  ...
  return (
    <div className="px-10 py-8 max-w-[720px]">
      <div className="flex items-start justify-between mb-8">
        <div>
          <h1 className="text-[20px] font-semibold text-ink-1 mb-1">API keys</h1>
          <p className="text-[13px] text-ink-3">Keys for the Agent Context API and external integrations.</p>
        </div>
        <button
          ...
          >
            <div className="flex items-start gap-2 mb-3">
              ...
              <div>
                ...
                <p className="text-[11.5px] text-ink-3 mt-0.5">Copy this key â€” it won't be shown again.</p>
              </div>
              <button onClick={() => setNewKey(null)} className="ml-auto text-ink-3 hover:text-ink-2">
                <X className="w-4 h-4" />
              </button>
            </div>
            <div className="flex items-center gap-2 p-3 rounded-lg bg-canvas border border-line font-mono text-[12px] text-ink-2">
      ...
      <AnimatePresence>
        {creating && (
          ...
          >
            ...
            >
              <h3 className="text-[16px] font-semibold text-ink-1 mb-5">Create API key</h3>
              <div className="mb-5">
              ...
              <div className="flex gap-3">
                <button onClick={() => setCreating(false)} className="flex-1 h-10 rounded-xl border border-line text-[13px] text-ink-3 hover:text-ink-2 hover:border-line-strong transition-all">Cancel</button>
      ...
      <div className="space-y-3">
        {apiKeys.map((key, i) => (
          <motion.div
            ...
            className="rounded-lg border border-line bg-surface p-5 group hover:border-line-strong transition-colors"
          >
            <div className="flex items-start justify-between mb-3">
              ...
              <button
                ...
                className="p-2 rounded-lg text-ink-4 hover:text-red-400 hover:bg-red-500/10 opacity-0 group-hover:opacity-100 transition-all"
              >
                <Trash2 className="w-3.5 h-3.5" />
              </button>
            </div>
            <div className="flex items-center gap-2 p-3 rounded-lg bg-canvas border border-line font-mono text-[12px]">
              <span className="text-ink-3 flex-1">
                {revealed === key.id ? `${key.prefix}${'x'.repeat(32)}` : `${key.prefix}${'â€¢'.repeat(20)}`}
              </span>
              <button onClick={() => setRevealed(revealed === key.id ? null : key.id)} className="text-ink-4 hover:text-ink-2 transition-colors">
                {revealed === key.id ? <EyeOff className="w-3.5 h-3.5" /> : <Eye className="w-3.5 h-3.5" />}
              </button>
              <button onClick={() => handleCopy(`${key.prefix}${'x'.repeat(32)}`, key.id)} className="text-ink-4 hover:text-primary transition-colors">
