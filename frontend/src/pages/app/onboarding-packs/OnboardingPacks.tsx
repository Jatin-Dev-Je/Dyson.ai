  ...
  return (
    <div className="px-8 py-7 max-w-[1100px] mx-auto">
      <div className="flex items-start justify-between mb-8">
        <div>
          <h1 className="text-[22px] font-semibold text-ink-1 mb-1">Onboarding Packs</h1>
          <p className="text-[13.5px] text-ink-3">Auto-generated context packs for new team members.</p>
        </div>
        <button
          onClick={() => setGenerating(true)}
          className="flex items-center gap-2 px-4 py-2.5 rounded-xl bg-primary text-[13px] font-medium text-white hover:bg-primary/90 transition-all shadow-sm active:scale-[0.98]"
        >
      ...
      {generating && (
        ...
        >
          <motion.div
            ...
            className="w-full max-w-md rounded-2xl border border-line bg-canvas p-7 shadow-[0_40px_80px_rgba(0,0,0,0.6)]"
          >
            <h3 className="text-[17px] font-semibold text-ink-1 mb-1.5">Generate context pack</h3>
            <p className="text-[13px] text-ink-3 mb-6">Dyson will build a personalised context pack for a new team member.</p>
            <div className="space-y-3 mb-6">
              <div>
                <label className="block text-[12px] font-medium text-ink-2 mb-1.5">New hire name</label>
                <input placeholder="Jordan Lee" className="w-full h-10 px-3.5 rounded-xl border border-line bg-subtle text-[13px] text-ink-1 placeholder:text-ink-4 outline-none focus:border-primary/50 focus:ring-2 focus:ring-primary/10 transition-all" />
              </div>
              <div>
                <label className="block text-[12px] font-medium text-ink-2 mb-1.5">Team</label>
                <select className="w-full h-10 px-3.5 rounded-xl border border-line bg-canvas text-[13px] text-ink-2 outline-none focus:border-primary/50 transition-all appearance-none">
                  {['Backend', 'Platform', 'Infra', 'Frontend', 'Data'].map(t => <option key={t}>{t}</option>)}
                </select>
              </div>
            </div>
            <div className="flex gap-3">
              <button onClick={() => setGenerating(false)} className="flex-1 h-10 rounded-xl border border-line text-[13px] text-ink-3 hover:text-ink-2 hover:border-line-strong transition-all">
                Cancel
              </button>
      ...
      <div className="grid grid-cols-3 gap-3 mb-7">
        {[
          ...
          <div key={i} className="rounded-lg border border-line bg-surface px-5 py-4">
            <p className="text-[22px] font-semibold text-ink-1 mb-0.5">{s.value}</p>
          ...
          >
            <Link to={`/app/onboarding-packs/${pack.id}`}>
              <motion.div
                whileHover={{ y: -2 }}
                className={cn(
                  'rounded-xl border border-line bg-surface p-6 cursor-pointer group',
                  'hover:border-line-strong hover:bg-canvas transition-all duration-200 hover:shadow-[0_8px_32px_rgba(0,0,0,0.4)]'
                )}
              >
                <div className="flex items-start justify-between mb-5">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-[14px] font-medium text-ink-1 group-hover:text-ink-1 transition-colors">{pack.name}</p>
                ...
                <div className="flex items-center gap-5 mb-5">
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-[12px] text-ink-3">{pack.decisions} decisions</span>
                  </div>
                  <div className="flex items-center gap-1.5">
                    <Clock className="w-3.5 h-3.5 text-ink-3" />
                    <span className="text-[12px] text-ink-3">Joined {pack.joined}</span>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-1 text-[12px] text-ink-3 group-hover:text-primary transition-colors">
