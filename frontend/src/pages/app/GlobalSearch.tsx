...
export default function GlobalSearch() {
  ...
  useEffect(() => {
    if (!query.trim()) { setResults(null); setSourceFilter('all'); return }
    const t = setTimeout(() => { void doSearch(query) }, 350)
    return () => clearTimeout(t)
  }, [query, doSearch])

  function resolveSource(source: string | undefined): 'slack' | 'github' | 'notion' | 'meeting' | 'linear' {
    if (!source) return 'linear'
  ...
  const displayed = sourceFilter === 'all'
    ? (results ?? [])
    : (results ?? []).filter(r => resolveSource(r.source) === sourceFilter)

  return (
    <div className="max-w-[760px] mx-auto px-8 py-8">
      <h1 className="text-[22px] font-semibold text-ink-1 -tracking-wide mb-6">Search</h1>
      ...
      <div className="relative mb-4">
        ...
        <input
          ...
          className="w-full h-11 pl-10 pr-12 bg-surface border border-line rounded-xl text-[14px] text-ink-1 placeholder:text-ink-4 outline-none focus:border-primary/50 focus:ring-2 focus:ring-primary/10 transition-all shadow-sm"
        />
      ...
      {/* Source filter chips — only shown when there's a query */}
      {query && (
      ...
      {error && (
        <div className="mb-4 px-3 py-2.5 rounded-lg bg-red-50 border border-red-200 text-[13px] text-danger">
          {error}
        </div>
      )}
      ...
      {!query ? (
          ...
          <div className="space-y-0.5">
            {recent.map((r, i) => (
              <button key={i} onClick={() => setQuery(r)}
                className="w-full flex items-center gap-3 px-3 py-2.5 rounded-lg text-[13px] text-ink-2 hover:bg-subtle hover:text-ink-1 transition-all text-left group">
        ...
        displayed.length === 0 ? (
          <div className="text-center py-16">
            ...
            <p className="text-[14px] font-medium text-ink-2 mb-1">No results for "{query}"</p>
            <p className="text-[12.5px] text-ink-4">
              {sourceFilter !== 'all'
                ? `Try removing the ${SOURCE_FILTERS.find(f => f.value === sourceFilter)?.label} filter`
                : 'Try a different keyword or connect more sources'}
            </p>
          </div>
        ) : (
          <div className="space-y-1.5">
            {displayed.map(r => (
              <a key={r.id} href={r.sourceUrl ?? '#'} target={r.sourceUrl ? '_blank' : undefined} rel="noreferrer"
                className="flex items-center gap-4 px-4 py-3.5 bg-surface border border-line rounded-xl hover:border-line-strong hover:shadow-sm transition-all group cursor-pointer">
                ...
                <div className="flex items-center gap-3 flex-shrink-0">
                  <SourcePill source={resolveSource(r.source)} />
                  {r.confidence !== undefined && <ConfidenceBadge confidence={r.confidence} />}
